...
class DNSManager:
    ...
    def __init__(self, router_logger, packet_writer, router_ipv6_ll):
        ...
        self._stats = {
            ...
            "upstream_probe_ok": 0,
            "upstream_probe_fail": 0,
            "fallback_question_match": 0,
            "terminal_reply_suppressed": 0,
        }
    ...
    def stop(self):
        ...
        if not self._probe_thread or not self._probe_thread.is_alive():
            self._elog("lifecycle", "Stop skipped because probe thread is not running.", ["🌙", "📭"])
            return
    ...
    def handle_query(self, packet: Packet, inbound_iface: str) -> bool:
        ...
        self._elog(
            "forward",
            f"Forward q={qname} type={qtype} chosen={chosen}{' hedge' if len(chosen) > 1 else ''}",
            ["➡️", "🌐", "📤"]
        )
    ...
    def _send_response_to_client(
        ...
        *,
        ...
        IPv6: source = router_ipv6_ll ONLY (link-local) unless router_ipv6_out exists.

        Reply path preference:
          1) explicit _dns_inbound_iface
          2) sniffed_on
        """
        ...
        if IP in original_request and IP in resp:
            ...
            resp[UDP].dport = int(original_request[UDP].sport)
            resp[DNS].id = original_request[DNS].id
            src_v6 = self.router_ipv6_link_local_out or self.router_ipv6_out or self.router_ipv6_ll
            if src_v6:
                resp[IPv6].src = str(src_v6).split("%", 1)[0]
            resp[UDP].sport = 53
            self._normalize_checksums(resp)
    ...
    def _ttl_from_response(self, dns, *, fallback: Optional[int] = None) -> int:
        ...
        try:
            ...
            for sect in ("an", "ns", "ar"):
                rr = getattr(dns, sect, None)
                seen = 0
                while rr is not None and seen < 128:
...
class UplinkManager(_SmartManagerBase):
    """
    Self-healing multi-uplink manager.

    Main goals:
      - keep the host/router online on Wi-Fi for long-running sessions
      - detect the Windows "connected but no internet" state fast
      - repair route / DNS / DHCP / adapter drift without waiting for a manual restart
      - keep ARP / NAT / packet writer / gateway manager in sync after repair or failover
      - avoid constant route flapping while still recovering aggressively when needed
    """
    ...
    def __init__(self, router: Any, gateway_manager: Optional[Any] = None):
        ...
        self._prefs = {
            ...
            "route_scrub_interval_sec": 60.0,
            "wifi_preserve_interval_sec": 20.0,
            "dns_cache_ttl_sec": 90.0,
            "gateway_cache_ttl_sec": 45.0,
            "adapter_cache_ttl_sec": 15.0,
            "min_probe_gap_sec": 6.0,
            "dns_probe_interval_sec": 18.0,
            "passive_signal_ttl_sec": 35.0,
            "max_probe_candidates_per_loop": 2,
            "activation_cooldown_sec": 20.0,

            # repair escalation
            "repair_failure_threshold": 2,
            "dhcp_repair_failure_threshold": 4,
            "adapter_reset_failure_threshold": 7,
            "repair_cooldown_sec": 20.0,
            "dns_reset_cooldown_sec": 45.0,
            "dhcp_renew_cooldown_sec": 90.0,
            "adapter_bounce_cooldown_sec": 240.0,

            # DNS behavior
            "fallback_dns_servers": list(self.DEFAULT_FALLBACK_DNS),
            "dns_probe_name": "www.msftconnecttest.com",
        }
    ...
    def _health_loop(self) -> None:
        while not self._stop_event.wait(self._prefs["health_interval_sec"]):
            try:
                ...
                self._maybe_repair_active_uplink()
                ...
                self._say(
                    "uplink_health_error",
                    f"uplink loop hit {type(e).__name__}: {e}",
                    ["⚠️", "🧩"],
                )
    ...
    def _discover_candidates(self) -> None:
        ...
        for iface_full, cfg in interfaces_config.items():
            ...
            if (not self._is_ipv4(gateway_ip)) or self._is_cache_expired(
                now,
                self._gateway_cache.get(friendly),
                self._prefs["gateway_cache_ttl_sec"],
            ):
    ...
    def _refresh_candidates_health(self) -> None:
        ...
        for idx, cand in enumerate(candidates):
            ...
            do_dns_probe = self._should_probe_dns(cand, now, idx)
            ...
            dns_ok = self._dns_health_from_recent(cand, now, default=cand.dns_ok)
            public_ok = self._public_health_from_passive(cand, now, default=cand.public_ok)

            if do_gateway_probe:
                gateway_ok = self._probe_gateway(cand)

            if gateway_ok and do_dns_probe:
                dns_ok = self._probe_dns(cand)
            elif not gateway_ok:
                dns_ok = False

            if do_public_probe:
                public_ok = self._probe_public(cand, dns_ok=dns_ok)
            ...
            with self._candidate_lock:
                ...
                real.dns_ok = bool(dns_ok)
                real.public_ok = bool(public_ok)

                if real.gateway_ok:
                    real.consecutive_gateway_failures = 0
                elif do_gateway_probe:
                    real.consecutive_gateway_failures += 1

                if real.dns_ok:
                    real.consecutive_dns_failures = 0
                elif do_dns_probe:
                    real.consecutive_dns_failures += 1

                if real.public_ok:
                    real.consecutive_public_failures = 0
                elif do_public_probe:
                    real.consecutive_public_failures += 1

                if real.public_ok or real.dns_ok or real.gateway_ok:
                    real.last_ok = now
                    real.ok_count += 1
    ...
    def _should_probe_dns(self, cand: UplinkCandidate, now: float, idx: int) -> bool:
        base_gap = float(self._prefs["dns_probe_interval_sec"])
        if cand.iface_full == self.active_uplink_full:
            return (now - cand.last_dns_probe) >= base_gap
        if cand.iface_friendly in self._prefs["preferred_iface_names"]:
            return (now - cand.last_dns_probe) >= (base_gap * 1.25)
        return idx < 2 and (now - cand.last_dns_probe) >= (base_gap * 1.75)
    ...
    def _should_probe_public(self, cand: UplinkCandidate, now: float, idx: int) -> bool:
        ...
        return idx < int(self._prefs["max_probe_candidates_per_loop"]) and (
            (now - cand.last_public_probe) >= (base_gap * 3.0)
        )
    ...
    def _dns_health_from_recent(self, cand: UplinkCandidate, now: float, default: bool) -> bool:
        ttl = float(self._prefs["passive_signal_ttl_sec"])
        if (now - cand.last_passive_public_at) <= ttl:
            return True
        if cand.dns_ok and (now - cand.last_dns_probe) <= max(ttl, float(self._prefs["dns_probe_interval_sec"]) * 2.0):
            return True
        return bool(default)
    ...
    def _probe_gateway(self, cand: UplinkCandidate) -> bool:
        ...
        # We no longer trust "gateway is in the same subnet" by itself.
        # It is only a weak hint, not proof of internet health.
        if not self._gateway_looks_routable(cand):
            cand.gateway_rtt_ms = None
            return False

        # Prefer a DNS-style probe against the gateway first if it acts like a resolver.
        ok, rtt_ms = self._timed_udp_dns_probe(
            bound_ip=cand.ip,
            dns_server=cand.gateway_ip,
            hostname=self._prefs["dns_probe_name"],
            timeout=1.0,
        )
        if ok:
        ...
        # Fallback to quick TCP connect attempts.
        for port in list(self._prefs["gateway_probe_ports"])[:3]:
            ok, rtt_ms = self._timed_tcp_probe(
                bound_ip=cand.ip,
                host=cand.gateway_ip,
                port=port,
                timeout=0.9,
            )
            if ok:
    ...
    def _probe_dns(self, cand: UplinkCandidate) -> bool:
        cand.last_dns_probe = time.time()
        cand.dns_rtt_ms = None

        if not cand.link_up or not cand.ip:
            return False

        servers: list[str] = []
        for item in list(cand.dns_servers or []):
            if self._is_ipv4(item) and item not in servers:
                servers.append(item)

        if self._is_ipv4(cand.gateway_ip) and cand.gateway_ip not in servers:
            servers.append(cand.gateway_ip)

        for item in list(self._prefs.get("fallback_dns_servers") or []):
            if self._is_ipv4(item) and item not in servers:
                servers.append(item)
        ...
        for dns_server in servers[:4]:
            ...
            if ok:
                cand.dns_rtt_ms = rtt_ms
                return True

        return False

    def _probe_public(self, cand: UplinkCandidate, dns_ok: bool = False) -> bool:
        if not cand.link_up or not cand.ip:
        ...
        # DNS can still be useful as a softer signal, but not enough by itself to say
        # the whole internet path is healthy.
        cand.public_rtt_ms = None
        return bool(False)
    ...
    def _maybe_repair_active_uplink(self) -> None:
        if not self.active_uplink_full:
            return

        with self._candidate_lock:
            cand = self.candidates.get(self.active_uplink_full)

        if cand is None:
            return

        if cand.public_ok:
            cand.metadata["repair_stage"] = 0
            return

        if not cand.link_up:
            return

        threshold = int(self._prefs["repair_failure_threshold"])
        should_repair = (
            cand.consecutive_public_failures >= threshold
            or (cand.gateway_ok and cand.consecutive_dns_failures >= threshold)
            or (cand.dns_ok and cand.consecutive_public_failures >= threshold)
            or (cand.consecutive_gateway_failures >= threshold)
        )
        ...
        now = time.time()
        if (now - cand.last_repair_at) < float(self._prefs["repair_cooldown_sec"]):
            return

        self._repair_active_uplink(cand)

    def _repair_active_uplink(self, cand: UplinkCandidate) -> None:
        now = time.time()
        cand.last_repair_at = now
        cand.repair_count += 1

        stage = int(cand.metadata.get("repair_stage", 0))
        if cand.consecutive_public_failures >= int(self._prefs["adapter_reset_failure_threshold"]):
            stage = 3
        elif cand.consecutive_public_failures >= int(self._prefs["dhcp_repair_failure_threshold"]):
            stage = max(stage, 2)

        self._say(
            "uplink_repair_begin",
            f"repairing active uplink {cand.iface_friendly} "
            f"(gw_fail={cand.consecutive_gateway_failures}, dns_fail={cand.consecutive_dns_failures}, public_fail={cand.consecutive_public_failures}, stage={stage})",
            ["🛠️", "📶", "🌐"],
        )

        self._invalidate_iface_caches(cand.iface_friendly)
        self._refresh_active_candidate_runtime(cand)

        # Always do the cheap/safe repair steps first.
        self._repair_host_routes()
        self._force_reapply_active_uplink(cand)
        self._preserve_wifi_link()

        if stage <= 0:
            self._repair_dns_for_iface(cand, fallback_only=False)
        elif stage == 1:
            self._repair_dns_for_iface(cand, fallback_only=True)
        elif stage == 2:
            self._renew_dhcp_for_iface(cand)
        else:
            self._bounce_adapter(cand)

        time.sleep(1.5 if stage < 2 else 4.0)
        ...
        with self._candidate_lock:
            refreshed = self.candidates.get(cand.iface_full)

        if refreshed and refreshed.public_ok:
            refreshed.metadata["repair_stage"] = 0
            self._say(
                "uplink_repair_ok",
                f"recovered internet on {refreshed.iface_friendly} {refreshed.ip}",
                ["✅", "🌍", "📶"],
                cooldown=0.0,
            )
        else:
            ...
            if refreshed:
                refreshed.metadata["repair_stage"] = next_stage
            self._say(
                "uplink_repair_pending",
                f"{cand.iface_friendly} still degraded after repair; next stage={next_stage}",
                ["⚠️", "🔁", "🌐"],
            )

    def _repair_host_routes(self) -> None:
        if self._prefs["remove_public_host_routes"]:
            self._scrub_public_host_routes()
        ...
        gm = self.gateway_manager or getattr(self.router, "gateway_manager", None)
        if gm is not None:
            try:
    ...
    def _repair_dns_for_iface(self, cand: UplinkCandidate, fallback_only: bool) -> None:
        now = time.time()
        last_dns_fix = float(cand.metadata.get("last_dns_fix_at", 0.0))
        if (now - last_dns_fix) < float(self._prefs["dns_reset_cooldown_sec"]):
            return

        alias = self._ps_quote(cand.iface_friendly)
        fallback_dns = [
            ip for ip in list(self._prefs.get("fallback_dns_servers") or [])
            if self._is_ipv4(ip)
        ]
        if not fallback_dns:
            fallback_dns = list(self.DEFAULT_FALLBACK_DNS)

        self._say(
            "uplink_dns_repair",
            f"repairing DNS on {cand.iface_friendly} "
            f"({'fallback' if fallback_only else 'reset+fallback'})",
            ["🧠", "📡", "🩹"],
        )
        ...
        if not fallback_only:
            self._run_subprocess(
                [
                    ...
                    "Bypass",
                    "-Command",
                    f"Set-DnsClientServerAddress -InterfaceAlias '{alias}' -ResetServerAddresses -ErrorAction SilentlyContinue",
                ],
                timeout=8.0,
            )
            self._run_subprocess(["ipconfig", "/flushdns"], timeout=8.0)

        quoted_dns = ",".join(f"'{self._ps_quote(ip)}'" for ip in fallback_dns)
        self._run_subprocess(
            [
                "powershell.exe",
                "-NoProfile",
                ...
                "Bypass",
                "-Command",
                f"Set-DnsClientServerAddress -InterfaceAlias '{alias}' -ServerAddresses @({quoted_dns}) -ErrorAction SilentlyContinue",
            ],
            timeout=8.0,
        )

        cand.metadata["last_dns_fix_at"] = now
        self._invalidate_iface_caches(cand.iface_friendly)

    def _renew_dhcp_for_iface(self, cand: UplinkCandidate) -> None:
        now = time.time()
        last_dhcp = float(cand.metadata.get("last_dhcp_renew_at", 0.0))
        if (now - last_dhcp) < float(self._prefs["dhcp_renew_cooldown_sec"]):
            return

        friendly = cand.iface_friendly
        alias = self._ps_quote(friendly)

        self._say(
            "uplink_dhcp_repair",
            f"renewing DHCP on {friendly}",
            ["🔄", "📶", "🧰"],
        )

        # Put the adapter back on DHCP before renewing.
        self._run_subprocess(
            [
                "powershell.exe",
                "-NoProfile",
                ...
                "Bypass",
                "-Command",
                f"Set-NetIPInterface -InterfaceAlias '{alias}' -Dhcp Enabled -ErrorAction SilentlyContinue",
            ],
            timeout=8.0,
        )

        self._run_subprocess(["ipconfig", "/release", friendly], timeout=20.0)
        self._run_subprocess(["ipconfig", "/renew", friendly], timeout=25.0)

        cand.metadata["last_dhcp_renew_at"] = now
        self._invalidate_iface_caches(cand.iface_friendly)

    def _bounce_adapter(self, cand: UplinkCandidate) -> None:
        now = time.time()
        last_bounce = float(cand.metadata.get("last_adapter_bounce_at", 0.0))
        if (now - last_bounce) < float(self._prefs["adapter_bounce_cooldown_sec"]):
            return

        alias = self._ps_quote(cand.iface_friendly)

        self._say(
            "uplink_adapter_bounce",
            f"bouncing adapter {cand.iface_friendly} as last resort",
            ["💥", "📶", "♻️"],
        )
        ...
        cand.metadata["last_adapter_bounce_at"] = now
        self._invalidate_iface_caches(cand.iface_friendly)

    def _force_reapply_active_uplink(self, cand: UplinkCandidate) -> None:
        try:
            r = self.router
    ...
    def _refresh_active_candidate_runtime(self, cand: UplinkCandidate) -> None:
        info = self._get_cached_ipv4_info_for_adapter(cand.iface_friendly)
        if info:
            cand.ip = info.get("ip")
            cand.netmask = info.get("netmask")
            cand.network = info.get("network")
        cand.link_up = self._iface_link_up(cand.iface_friendly)
        cand.gateway_ip = self._get_cached_gateway_for_iface(cand.iface_friendly, cand.ip)
        cand.dns_servers = self._get_windows_dns_servers(cand.iface_friendly)

    def _invalidate_iface_caches(self, iface_friendly: Optional[str]) -> None:
        if not iface_friendly:
            return
        self._adapter_cache.pop(iface_friendly, None)
        self._gateway_cache.pop(iface_friendly, None)
        self._dns_cache.pop(iface_friendly, None)
    ...
    def _score_candidate(self, cand: UplinkCandidate) -> float:
        ...
        score -= min(cand.fail_count * 4.0, 30.0)
        score -= min(cand.consecutive_gateway_failures * 3.0, 18.0)
        score -= min(cand.consecutive_dns_failures * 3.0, 18.0)
        score -= min(cand.consecutive_public_failures * 4.0, 24.0)
        score -= min(cand.repair_count * 1.0, 8.0)

        if cand.public_rtt_ms is not None:
            score -= min(cand.public_rtt_ms / 25.0, 10.0)
        if cand.dns_rtt_ms is not None:
            score -= min(cand.dns_rtt_ms / 30.0, 6.0)
    ...
    def _choose_best_candidate(self) -> Optional[UplinkCandidate]:
        ...
        if active and active.link_up and (active.dns_ok or active.gateway_ok or (time.time() - active.last_ok) < 60.0):
            return active
    ...
    def _activate_candidate(self, cand: UplinkCandidate) -> bool:
        ...
        extra = "public-ready" if cand.public_ok else ("dns-ready" if cand.dns_ok else "gateway-only")
        self._say(
            "uplink_activated",
    ...
    def _preserve_wifi_link(self) -> None:
        ...
        for cand in self._ordered_candidates_for_health():
            ...
            self._say(
                "wifi_preserved",
                f"keeping {cand.iface_friendly} online as a live preferred uplink candidate",
                ["📶", "🫶", "🔒"],
            )
            return
    ...
    def _enforce_netroute_safe_mode(self) -> None:
        ...
        if changed:
            self._say(
                "uplink_safe_mode",
                "disabled NetRoute default-route sync / metric tuning for safer host internet",
                ["🛡️", "🚧"],
            )
...
""",
        ...
        if not servers:
            proc = self._run_subprocess(["ipconfig", "/all"], timeout=4.0)
            if proc and proc.returncode == 0:
    ...
    def _dns_query_packet(self, hostname: str, txid: int) -> bytes:
        out = bytearray()
        out += int(txid & 0xFFFF).to_bytes(2, "big")
        out += b"\x01\x00"                  # standard query, recursion desired
        out += b"\x00\x01"                  # QDCOUNT
        out += b"\x00\x00\x00\x00\x00\x00"  # AN/NS/AR counts

        for label in str(hostname).strip(".").split("."):
            enc = label.encode("ascii", errors="ignore")[:63]
            out.append(len(enc))
            out += enc
        out += b"\x00"       # end name
    ...
    def _timed_udp_dns_probe(
        ...
        *,
        ...
        try:
            ...
            payload = self._dns_query_packet(hostname, txid)
            s.sendto(payload, (dns_server, 53))
            resp, _addr = s.recvfrom(1024)

            if len(resp) < 12:
                return False, None

            if int.from_bytes(resp[0:2], "big") != txid:
                return False, None
            ...
            ok = qr and rcode == 0 and ancount >= 0
            return ok, ((time.perf_counter() - t0) * 1000.0 if ok else None)
    ...
    def _get_cached_ipv4_info_for_adapter(self, iface_friendly: str) -> Optional[dict]:
        ...
        try:
            ...
            if info and info.get("ip") and info.get("netmask") and not info.get("network"):
                info["network"] = ipaddress.ip_network(
                    f"{info['ip']}/{info['netmask']}",
                    strict=False,
                )
        except Exception:
            info = None
    ...
    def _get_cached_gateway_for_iface(
        self,
        iface_friendly: str,
    ...
    def _container_contains_any(
        ...
        for attr in (
            ...
            try:
                if hasattr(obj, attr) and self._container_contains_any(
                    getattr(obj, attr),
                    tokens,
                    depth - 1,
                    max_items,
...
class _Peer:
    ...
    rx_bytes: int = 0
    tx_bytes: int = 0
    last_frame_rx_ts: float = 0.0
    last_frame_tx_ts: float = 0.0
...
class HyperVRouterManager:
    ...
    This rewrite keeps the public API/signatures the same, while improving:
      - frame compatibility with older HVRM3 peers
      - remote frame receive/processing logs on BOTH machines
      - explicit peer discovered / peer changed / peer offline logs
      - ack flow so sender machine knows whether remote queued/processed the frame
      - hello reply over data socket for better direct peer awareness
      - select()-based socket receive loop
      - send-only discovery socket so one local bind problem does not kill discovery adverts
      - safer network queue metadata and per-peer stats
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._discovery_sock: Optional[socket.socket] = None           # recv multicast hello
        self._discovery_tx_sock: Optional[socket.socket] = None        # send-only hello
        self._data_sock: Optional[socket.socket] = None                # recv/send frames/acks/replies
        ...
        self._frame_ack_timeout_sec = max(6.0, self.peer_timeout_sec + 3.0)
    ...
    def start(self) -> None:
        ...
        if self._data_sock is None:
            ...
            self._log(
                "started with discovery receive disabled but discovery send active; peer replies may still arrive over data socket",
                ["⚠️", "📡", "↪️"],
            )
    ...
    def _sender_worker_loop(self, state: _LocalSender) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                if pkt is None:
                    state.drop_count += 1
                    if qf.from_remote and qf.reply_ip and qf.reply_port:
                        self._send_frame_ack(
                            dst_ip=qf.reply_ip,
                            dst_port=qf.reply_port,
                            dst_node_id=qf.src_node_id,
                            frame_id=qf.frame_id,
                            status="decode_failed",
                            detail=f"local sender={state.sender_id} could not decode raw frame",
                        )
                    continue
                ...
                if qf.from_remote:
                    self._log(
                        f"processed remote frame frame_id={qf.frame_id} trace={qf.trace_id or '-'} "
                        f"src_node={qf.src_node_id or '?'} src_host={qf.src_host_name or '?'} "
                        f"via sender={state.sender_id} sender_kind={state.kind} "
                        f"protocol={qf.protocol_tag or '?'} len={len(qf.raw)}",
                        ["📥", "✅", "🏠", "🧵"],
                    )
                    if qf.reply_ip and qf.reply_port:
                        self._send_frame_ack(
                            dst_ip=qf.reply_ip,
                            dst_port=qf.reply_port,
                            dst_node_id=qf.src_node_id,
                            frame_id=qf.frame_id,
                            status="processed",
                            detail=f"local sender={state.sender_id} kind={state.kind} processed frame",
                        )
                ...
                if qf.from_remote and qf.reply_ip and qf.reply_port:
                    self._send_frame_ack(
                        ...
                        frame_id=qf.frame_id,
                        status="send_failed",
                        detail=f"local sender={state.sender_id} failed: {type(e).__name__}: {e}",
                    )
    ...
    def _enqueue_local(self, state: _LocalSender, raw: bytes) -> bool:
        try:
            state.send_q.put_nowait(_QueuedLocalFrame(raw=raw))
            return True
        except queue.Full:
            try:
    ...
    def _enqueue_local_remote_frame(
        self,
        state: _LocalSender,
        *,
        raw: bytes,
    ...
    def _choose_endpoint(self, packet, inbound_iface: str, protocol_tag: str, raw: bytes, endpoints):
        ...
        has_remote = any(kind == "remote" for kind, _ in endpoints)
        ...
        def _score(item):
            ...
            if kind == "local":
                ...
                # If there are live remote peers, bias away from staying fully local
                # so cross-host forwarding actually happens.
                if has_remote:
                    score -= 2
                return score
            ...
            if ingress_kind == "hyperv":
                score += 8
            elif ingress_kind in {"wintun", "windivert"} and obj.sender_kind == "hyperv":
                score += 6
        ...
        # Top-score first, but still stable across flows among similarly-ranked endpoints.
        top_score = _score(endpoints[0])
        top = [ep for ep in endpoints if _score(ep) == top_score]
    ...
    def _open_discovery_sender_socket(self) -> Optional[socket.socket]:
        try:
            ...
            try:
                ds.setsockopt(socket.IPPROTO_IP, socket.IP_MULTICAST_TTL, 2)
            except Exception:
                pass
            try:
    ...
    def _open_discovery_socket(self) -> Optional[socket.socket]:
        try:
            ...
            if self._is_addr_in_use_error(e):
                self._log(
                    f"discovery bind in use on *:{self.discovery_port}; receive disabled for this instance but send-only discovery remains active",
                    ["⚠️", "📡", "🧯"],
                )
        ...
        while True:
            ...
            try:
                ...
                if mtype == "frame":
                    ...
                    self._log(
                        f"sent remote frame frame_id={frame_id} trace={trace_id or '-'} "
                        f"to peer={peer_node_id or '?'} ip={item['ip']}:{item['port']} "
                        f"protocol={item.get('protocol_tag') or '?'} dst_sender={item.get('dst_sender_id') or '?'}",
                        ["📤", "🌐", "✅"],
                    )
                elif mtype == "ack":
                    self._log(
                        f"sent ack frame_id={frame_id} to peer={peer_node_id or '?'} "
                        f"ip={item['ip']}:{item['port']} status={status or '?'}",
                        ["↩️", "📨", "✅"],
                    )
                elif mtype == "hello_reply":
                    self._log(
                        f"sent direct hello-reply to peer={peer_node_id or '?'} ip={item['ip']}:{item['port']}",
                        ["🤝", "📡", "↪️"],
                    )
    ...
    def _handle_wire_message(self, raw: bytes, from_ip: str, from_port: int, which: str) -> None:
        ...
        # IMPORTANT:
        # older code only looked at node_id, but frame messages commonly carry src_node_id.
        ...
        if mtype in {"hello", "hello_ack"}:
            ...
            if mtype == "hello" and which == "discovery":
                self._send_hello_reply(msg, from_ip)
            return
    ...
    def _update_peer_from_hello(self, msg: dict, from_ip: str) -> None:
        ...
        with self._lock:
            ...
            old_ip = peer.listen_ip
            old_port = peer.data_port
            old_host = peer.host_name
            old_senders = set(peer.sender_ids)
            ...
            if (
                peer.listen_ip != old_ip
                or peer.data_port != old_port
                or peer.host_name != old_host
                or peer.sender_ids != old_senders
            ):
                changed = True

        if is_new:
            self._log(
                f"peer discovered node={node_id} host={peer.host_name} "
                f"listen={peer.listen_ip}:{peer.data_port} senders={sorted(peer.sender_ids)} "
                f"protocols={sorted(peer.allow_protocols)}",
                ["🤝", "🌐", "✅"],
            )
        elif changed:
            self._log(
                f"peer updated node={node_id} host={peer.host_name} "
                f"listen={peer.listen_ip}:{peer.data_port} senders={sorted(peer.sender_ids)}",
                ["🔄", "🌐", "📝"],
            )

    def _send_hello_reply(self, hello_msg: dict, from_ip: str) -> None:
        with self._sock_lock:
            has_data = self._data_sock is not None

        if not has_data:
            return

        reply_port = int(hello_msg.get("data_port") or 0)
        if reply_port <= 0:
            return
    ...
    def _handle_remote_frame(self, msg: dict, from_ip: str, from_port: int) -> None:
        ...
        src_node_id = str(msg.get("src_node_id") or msg.get("node_id") or "")
        src_host_name = str(msg.get("host_name") or src_node_id or "")
        src_sender_id = str(msg.get("src_sender_id") or "")
        dst_sender_id = str(msg.get("dst_sender_id") or "")
        protocol_tag = str(msg.get("protocol_tag") or "")
        inbound_iface = str(msg.get("inbound_iface") or "")
        trace_id = str(msg.get("trace_id") or frame_id or "")
        ...
        if self._seen_recently(f"remote:{src_node_id}:{frame_id}"):
            self._log(
                f"received duplicate remote frame frame_id={frame_id} trace={trace_id or '-'} "
                f"from node={src_node_id} ip={from_ip}:{from_port}; ignoring",
                ["♻️", "📥", "🧱"],
            )
            self._send_frame_ack(
                dst_ip=from_ip,
                ...
                frame_id=frame_id,
                status="duplicate",
                detail="duplicate remote frame ignored",
            )
            return
        ...
        self._log(
            f"received remote frame frame_id={frame_id} trace={trace_id or '-'} "
            f"from node={src_node_id} host={src_host_name or '?'} "
            f"ip={from_ip}:{from_port} protocol={protocol_tag or '?'} "
            f"dst_sender={dst_sender_id or '-'} len={len(raw)}",
            ["📥", "🌐", "✅"],
        )
        ...
        if sender is None:
            self._log(
                f"remote frame has no eligible local sender frame_id={frame_id} "
                f"src_node={src_node_id} protocol={protocol_tag or '?'}",
                ["⚠️", "📥", "🕳️"],
            )
            self._send_frame_ack(
                dst_ip=from_ip,
                ...
                dst_node_id=src_node_id,
                frame_id=frame_id,
                status="dropped_no_sender",
                detail=f"no eligible sender for protocol={protocol_tag or '?'}",
            )
            return

        ok = self._enqueue_local_remote_frame(
            sender,
            raw=raw,
            protocol_tag=protocol_tag,
            frame_id=frame_id,
            ...
            inbound_iface=inbound_iface,
            reply_ip=from_ip,
            reply_port=from_port,
        )
        if not ok:
            self._log(
                f"remote frame queue full frame_id={frame_id} src_node={src_node_id} "
                f"dst_sender={sender.sender_id}",
                ["⚠️", "📥", "📬", "🧯"],
            )
            self._send_frame_ack(
                dst_ip=from_ip,
                ...
                frame_id=frame_id,
                status="queue_full",
                detail=f"sender={sender.sender_id} queue full",
            )
            return

        self._log(
            f"accepted remote frame frame_id={frame_id} trace={trace_id or '-'} "
            f"src_node={src_node_id} queued_to={sender.sender_id} kind={sender.kind}",
            ["📥", "📬", "✅"],
        )
        self._send_frame_ack(
            dst_ip=from_ip,
            ...
            frame_id=frame_id,
            status="accepted",
            detail=f"queued to sender={sender.sender_id} kind={sender.kind}",
        )

    def _handle_frame_ack(self, msg: dict, from_ip: str, from_port: int) -> None:
        frame_id = str(msg.get("frame_id") or "")
        ack_from = str(msg.get("src_node_id") or msg.get("node_id") or "")
        ...
        self._log(
            f"received ack frame_id={frame_id} from node={ack_from or '?'} "
            f"ip={from_ip}:{from_port} status={status or '?'} detail={detail or '-'}",
            ["↩️", "📨", "🌐"],
        )
    ...
    def _choose_local_sender_for_remote(self, *, dst_sender_id: str, protocol_tag: str, raw: bytes) -> Optional[_LocalSender]:
        ...
        with self._lock:
            if dst_sender_id and dst_sender_id in self._senders:
                ...
                if state.enabled and now >= state.cooldown_until and (not protocol_tag or protocol_tag in state.allow_protocols):
                    return state
    ...
    def _enqueue_remote(
        ...
        *,
        ...
        trace_id = hashlib.blake2b(
            (frame_id + "|" + protocol_tag + "|" + inbound_iface).encode("utf-8"),
            digest_size=8,
        ).hexdigest()
        ...
        msg = {
            "magic": self.MAGIC,
            "type": "frame",
            # keep node_id for backward compatibility with older HVRM3 receivers
            "node_id": self.node_id,
            ...
            "frame_id": frame_id,
            "trace_id": trace_id,
            "reply_port": self._bound_data_port or self.data_port,
        ...
        with self._pending_lock:
            self._pending_frames[frame_id] = {
                ...
                "peer_ip": peer.listen_ip,
                "peer_port": peer.data_port,
                "protocol_tag": protocol_tag,
                "dst_sender_id": dst_sender_id,
        ...
        return self._enqueue_network_wire(
            raw=wire,
            ip=peer.listen_ip,
            port=peer.data_port,
            mtype="frame",
        ...
        *,
        ...
        try:
            self._net_tx_q.put_nowait({
                ...
                "protocol_tag": str(protocol_tag or ""),
                "dst_sender_id": str(dst_sender_id or ""),
            })
            return True
        except queue.Full:
            self._log(
                f"network queue full mtype={mtype} peer={peer_node_id or '?'} ip={ip}:{port}",
                ["⚠️", "📡", "🧯"],
            )
            return False
    ...
    def _send_frame_ack(
        ...
        *,
        ...
        msg = {
            "magic": self.MAGIC,
            "type": "ack",
            # keep node_id for compatibility with old parsers that only read node_id
            "node_id": self.node_id,
            "src_node_id": self.node_id,
            "segment_id": self.segment_id,
            "host_name": self.host_name,
    ...
    def _prune_peers(self) -> None:
        ...
        for node_id, host_name, ip, port in offline_logs:
            self._log(
                f"peer offline node={node_id} host={host_name} listen={ip}:{port}",
                ["⚠️", "🌐", "💤"],
            )
    ...
    def _prune_pending_frames(self) -> None:
        ...
        for info in expired:
            self._log(
                f"frame ack timeout frame_id={info.get('frame_id')} trace={info.get('trace_id') or '-'} "
                f"peer={info.get('peer_node_id') or '?'} ip={info.get('peer_ip')}:{info.get('peer_port')} "
...
class _WSGIServerWorker(threading.Thread):
    def __init__(
        self,
        app: Flask,
        host: str,
        ...
        super().__init__(daemon=True, name="PythonServerManager-WSGIWorker")
        self._app = app
        self._host = str(host)
        self._port = int(port)
    ...
    def run(self) -> None:
        ...
        try:
            ...
            with self._state_lock:
                self._health_state["worker_last_bound_at"] = time.time()
                self._health_state["last_bind_error"] = None
            self._ready.set()
            ...
            with self._state_lock:
                self._health_state["last_worker_exception"] = self._last_error
                if not self._bound_ok:
                    self._health_state["last_bind_error"] = self._last_error
            self._ready.set()
        finally:
            try:
...
class PythonServerManager:
    """
    Hardened, restartable Flask server manager for long router runtimes.

    What changed versus the fragile version:
    - threaded WSGI server instead of one single blocking serve_forever thread
    - supervisor that restarts the HTTP worker if it dies
    - background housekeeping/snapshot thread so packet/log ingestion does not do
      heavy pruning work in hot paths
    - cached stats endpoint to keep the dashboard responsive under traffic spikes
    - clean logger unwrapping support for router shutdown
    - bounded raw-packet retention with emergency pressure shedding
    """
    ...
    PREFIX_RE = __import__("re").compile(r"\[([^\[\]]+)\]")
    ...
    def __init__(
        ...
        *,
        ...
        client_stream_max_packets: int = 12000,
        client_stream_max_bytes: int = 512 * 1024 * 1024,
        client_stream_ttl_sec: float = 900.0,
    ) -> None:
        self.router = router
        self.router_logger = router_logger or getattr(router, "router_logger", None)
        self._router_log_passthrough = getattr(self.router_logger, "log_message", None) if self.router_logger else None
        ...
        self.max_raw_packet_bytes = max(0, int(max_raw_packet_bytes))
        self.raw_hex_preview_bytes = max(8, int(raw_hex_preview_bytes))
        self.client_stream_max_packets = max(256, int(client_stream_max_packets))
        self.client_stream_max_bytes = max(8 * 1024 * 1024, int(client_stream_max_bytes))
        self.client_stream_ttl_sec = max(30.0, float(client_stream_ttl_sec))

        self.started_at = time.time()
        self._lock = threading.RLock()
        self._stop_event = threading.Event()
        self._client_packet_cv = threading.Condition(self._lock)

        self._app: Optional[Flask] = None
        self._worker: Optional[_WSGIServerWorker] = None
        ...
        self._logs_by_prefix: Dict[str, Deque[Dict[str, Any]]] = defaultdict(lambda: deque(maxlen=self.max_logs_per_prefix))
        ...
        self._raw_packet_order: Deque[int] = deque(maxlen=max(self.max_packets * 2, 512))
        self._raw_packet_total_bytes = 0
        self._client_packet_meta: Deque[Dict[str, Any]] = deque(maxlen=self.client_stream_max_packets)
        self._client_packet_raw: Dict[int, Tuple[bytes, float]] = {}
        self._client_packet_order: Deque[int] = deque(maxlen=self.client_stream_max_packets * 2)
        self._client_packet_total_bytes = 0
        ...
        self._max_request_limit = 5000

        self._packet_counters: Dict[str, Any] = {
            "total_packets": 0,
            "total_bytes": 0,
            ...
            "client_packet_bytes": 0,
            "dropped_client_packets": 0,
            "dropped_client_bytes": 0,
            "by_topic": Counter(),
            "by_proto": Counter(),
            "by_l3": Counter(),
            "by_l4": Counter(),
        ...
        self._log_counters: Dict[str, Any] = {
            "total_logs": 0,
            "coalesced_logs": 0,
            "by_level": Counter(),
            "by_source": Counter(),
        ...
        self._health_state: Dict[str, Any] = {
            ...
            "last_worker_exception": None,
            "last_bind_error": None,
            "worker_last_bound_at": None,
            "supervisor_started_at": None,
            "worker_alive": False,
            "supervisor_alive": False,
            "housekeeping_alive": False,
            "consecutive_worker_failures": 0,
        }

        self._raw_packet_budget_bytes = 128 * 1024 * 1024 if self.max_raw_packet_bytes == 0 else max(
            self.max_raw_packet_bytes * 128,
    ...
    def start(self) -> None:
        with self._lock:
            ...
            self._health_state["supervisor_started_at"] = time.time()
            self._supervisor_thread = threading.Thread(target=self._supervisor_loop, daemon=True, name="PythonServerManager-Supervisor")
            self._housekeeping_thread = threading.Thread(target=self._housekeeping_loop, daemon=True, name="PythonServerManager-Housekeeping")
            self._supervisor_thread.start()
            self._housekeeping_thread.start()
        self.display_log(f"[PythonServer] Server supervisor started at http://{self.host}:{self.port}", source="PythonServer", level="info")
    ...
    def stop(self) -> None:
        ...
        if supervisor is not None:
            supervisor.join(timeout=2.5)
        if housekeeping is not None:
            housekeeping.join(timeout=2.5)
        self.display_log("[PythonServer] Server supervisor stopped.", source="PythonServer", level="info")

    def _supervisor_loop(self) -> None:
        backoff = 0.25
        while not self._stop_event.is_set():
            with self._lock:
                self._health_state["supervisor_alive"] = True
                worker = self._worker
                worker_dead = worker is None or not worker.is_alive()

            if worker_dead:
                new_worker = _WSGIServerWorker(
                    app=self._app if self._app is not None else self._create_flask_app(),
                    host=self.host,
                    port=self.port,
                ...
                new_worker.start()
                new_worker.ready_event.wait(timeout=2.0)
                time.sleep(0.10)

                if not new_worker.bound_ok and not self._stop_event.is_set():
                    with self._lock:
                        self._health_state["supervisor_restarts"] += 1
                        self._health_state["consecutive_worker_failures"] += 1
                    self.display_log(
                        "[PythonServer] HTTP worker failed to bind/start; supervisor will retry.",
                        source="PythonServer",
                        level="error",
                        extra={"last_error": new_worker.last_error},
                    )
                    time.sleep(min(backoff, 5.0))
                    backoff = min(backoff * 2.0, 5.0)
                    continue

                if new_worker.bound_ok:
                    with self._lock:
                        if self._health_state["supervisor_restarts"] > 0:
                            self.display_log(
                                "[PythonServer] HTTP worker recovered after restart.",
                                source="PythonServer",
                                level="warning",
                            )
                        self._health_state["consecutive_worker_failures"] = 0
                    backoff = 0.25

            with self._lock:
                self._health_state["worker_alive"] = bool(self._worker and self._worker.is_alive())
            time.sleep(0.5)

        with self._lock:
            self._health_state["worker_alive"] = False
            self._health_state["supervisor_alive"] = False

    def _housekeeping_loop(self) -> None:
        while not self._stop_event.is_set():
            now = time.time()
            with self._lock:
                self._health_state["housekeeping_alive"] = True
                self._prune_rate_window_locked(now)
                self._prune_packets_locked(now)
                self._prune_logs_locked(now)
                self._prune_events_locked(now)
                self._prune_raw_packets_locked(now)
                self._prune_client_packets_locked(now)
                self._prune_flows_locked(now)
                self._prune_recent_log_dupes_locked(now)
                self._trim_prefix_buckets_locked(now)
                self._cached_snapshot = self._build_dashboard_snapshot_locked(now)
                self._cached_snapshot_at = now
            time.sleep(self._housekeeping_interval_sec)
        with self._lock:
            self._health_state["housekeeping_alive"] = False
    ...
    def display_log(
        ...
        *,
        ...
        ex = dict(extra or {})
        ex.setdefault("full_prefix", " › ".join(prefixes) if prefixes else primary_prefix)
        ...
        *,
        ...
        def wrapped(*args: Any, **kwargs: Any) -> Any:
            ...
            try:
                ...
                try:
                    message = message_getter(args, kwargs, result, error) if message_getter else self._default_log_message_from_call(args, kwargs, result, error)
                    extra = extra_getter(args, kwargs, result, error) if extra_getter else {}
    ...
    def get_logs(
        ...
        *,
        ...
        limit = max(1, min(self._max_request_limit, int(limit)))
        with self._lock:
            items = list(self._logs_by_prefix.get(str(prefix), [])) if prefix else list(self._logs)
        ...
        with self._lock:
            ...
            for prefix, dq in self._logs_by_prefix.items():
                ...
                rows.append({
                    "prefix": prefix,
                    "count": int(len(dq)),
                    "oldest_id": int(dq[0].get("id") or 0),
                    "newest_id": int(dq[-1].get("id") or 0),
    ...
    def handle_packet(
        ...
        *,
        ...
        with self._lock:
            ...
            self._store_client_packet_locked(rec, raw)
            self._client_packet_cv.notify_all()
            return rec
    ...
    def get_client_packets(
        ...
        *,
        ...
        encoding = str(encoding or "base64").lower()
        if encoding not in {"base64", "hex"}:
            encoding = "base64"
        wait_ms = max(0, min(30000, int(wait_ms)))
        deadline = time.time() + (wait_ms / 1000.0)

        with self._client_packet_cv:
            while True:
                items = list(self._client_packet_meta)
                newest_id = int(items[-1].get("id") or 0) if items else 0
                if newest_id > after_id or wait_ms <= 0 or self._stop_event.is_set():
                    raw_map = dict(self._client_packet_raw) if include_raw else {}
                    break
                remaining = deadline - time.time()
                if remaining <= 0:
                    raw_map = {}
                    break
                self._client_packet_cv.wait(timeout=min(remaining, 1.0))
        ...
        for item in items:
            ...
            rec = dict(item)
            if include_raw:
                entry = raw_map.get(int(rec.get("id") or 0))
                if entry is not None:
                    raw = entry[0]
                    rec["client_raw_encoding"] = encoding
                    rec["client_raw_len"] = len(raw)
                    rec["client_raw"] = base64.b64encode(raw).decode("ascii") if encoding == "base64" else raw.hex()
            out.append(rec)
        ...
        oldest_id = int(items[0].get("id") or 0) if items else 0
        newest_id = int(items[-1].get("id") or 0) if items else 0
        return {
            "items": out,
            "count": len(out),
            "oldest_id": oldest_id,
            "newest_id": newest_id,
            "encoding": encoding if include_raw else None,
        }

    def get_client_packet_raw(self, packet_id: int) -> Optional[bytes]:
        with self._lock:
            entry = self._client_packet_raw.get(int(packet_id))
            return None if entry is None else bytes(entry[0])
    ...
    def get_packets(
        ...
        *,
        ...
        with self._lock:
            ...
            raw_map = dict(self._raw_packets) if include_raw or include_hex_preview else {}
        ...
        for item in items:
            ...
            if phase and str(item.get("phase") or "") != str(phase):
                continue
            rec = dict(item)
            if include_raw or include_hex_preview:
                entry = raw_map.get(int(rec.get("id") or 0))
                if entry is not None:
                    raw = entry[0]
                    if include_raw:
                        rec["raw_hex"] = raw.hex()
                    elif include_hex_preview:
                        rec["raw_hex_preview"] = raw[: self.raw_hex_preview_bytes].hex()
            out.append(rec)
    ...
    def get_dashboard_snapshot(self) -> Dict[str, Any]:
        with self._lock:
            if not self._cached_snapshot:
                self._cached_snapshot = self._build_dashboard_snapshot_locked(time.time())
                self._cached_snapshot_at = time.time()
            snap = json.loads(json.dumps(self._cached_snapshot))
            snap["server"]["snapshot_age_sec"] = round(max(0.0, time.time() - self._cached_snapshot_at), 3)
            return snap
    ...
    def _create_flask_app(self) -> Flask:
        ...
        def api_health() -> Response:
            ...
            return jsonify({
                "ok": bool(snap["server"]["http_worker_alive"] and snap["server"]["housekeeping_alive"]),
                "uptime_sec": snap["server"]["uptime_sec"],
                "host": self.host,
                "port": self.port,
                "supervisor_restarts": snap["server"]["supervisor_restarts"],
                "snapshot_age_sec": snap["server"]["snapshot_age_sec"],
        ...
        def api_logs() -> Response:
            ...
            items = self.get_logs(
                limit=self._int_arg("limit", 200, 1, self._max_request_limit),
        ...
        def api_packets() -> Response:
            items = self.get_packets(
                ...
                include_hex_preview=(request.args.get("include_hex_preview", "1") != "0"),
            )
            return jsonify({"items": items, "count": len(items)})

        @app.get("/api/client-packets")
        def api_client_packets() -> Response:
            payload = self.get_client_packets(
                limit=self._int_arg("limit", 100, 1, 2000),
                after_id=self._int_arg("after_id", 0, 0, self.SAFE_JS_INT_MAX),
                topic=request.args.get("topic"),
                iface=request.args.get("iface"),
                component=request.args.get("component"),
                phase=request.args.get("phase"),
                include_raw=(request.args.get("include_raw", "1") != "0"),
                encoding=request.args.get("encoding") or "base64",
                wait_ms=self._int_arg("wait_ms", 0, 0, 30000),
            )
            return jsonify(payload)

        @app.get("/api/client-packets/<int:packet_id>")
        def api_client_packet_one(packet_id: int) -> Response:
            rec = self.get_client_packet_record(packet_id)
            if rec is None:
                return Response(status=404)
            raw = self.get_client_packet_raw(packet_id)
            if raw is not None:
                rec["client_raw_encoding"] = "base64"
                rec["client_raw_len"] = len(raw)
                rec["client_raw"] = base64.b64encode(raw).decode("ascii")
            return jsonify(rec)

        @app.get("/api/client-packets/<int:packet_id>/raw")
        def api_client_packet_raw(packet_id: int) -> Response:
            raw = self.get_client_packet_raw(packet_id)
            if raw is None:
                return Response(status=404)
            return Response(raw, mimetype="application/octet-stream", headers={
                "X-Packet-Id": str(packet_id),
    ...
    def _build_dashboard_snapshot_locked(self, now: float) -> Dict[str, Any]:
        recent_packets = sum(1 for ts, _ in self._rate_window if (now - ts) <= self.packet_window_sec)
        recent_bytes = sum(size for ts, size in self._rate_window if (now - ts) <= self.packet_window_sec)
        p = self._packet_counters
        l = self._log_counters
        prefixes = self.get_log_prefixes(limit=100)
        return {
            "server": {
                "uptime_sec": round(max(0.0, now - self.started_at), 3),
                "host": self.host,
                "port": self.port,
                "http_worker_alive": bool(self._worker and self._worker.is_alive()),
                "supervisor_alive": bool(self._supervisor_thread and self._supervisor_thread.is_alive()),
                "housekeeping_alive": bool(self._housekeeping_thread and self._housekeeping_thread.is_alive()),
                "supervisor_restarts": int(self._health_state.get("supervisor_restarts") or 0),
                "consecutive_worker_failures": int(self._health_state.get("consecutive_worker_failures") or 0),
                "last_worker_exception": self._health_state.get("last_worker_exception"),
                "last_bind_error": self._health_state.get("last_bind_error"),
                "worker_last_bound_at": self._health_state.get("worker_last_bound_at"),
            },
            "packets": {
                "total_packets": p["total_packets"],
                "total_bytes": p["total_bytes"],
                "live_packet_records": len(self._packets),
                "packet_ring_fill_ratio": round(len(self._packets) / max(1, self.max_packets), 4),
                ...
                "raw_store_fill_ratio": round(self._raw_packet_total_bytes / max(1, self._raw_packet_budget_bytes), 4),
                ...
                "client_packet_count": p["client_packet_count"],
                "client_packet_bytes": p["client_packet_bytes"],
                "dropped_client_packets": p["dropped_client_packets"],
                "dropped_client_bytes": p["dropped_client_bytes"],
    ...
    def _default_log_message_from_call(
        ...
        if kwargs:
            try:
                return self._stringify_message(kwargs)
            except Exception:
                pass
        if error is not None:
            return f"wrapped log call raised: {type(error).__name__}: {error}"
    ...
    def _prune_rate_window_locked(self, now: float) -> None:
        cutoff = now - self.packet_window_sec
    ...
    def _prune_flows_locked(self, now: float) -> None:
        ...
        stale = [k for k, v in self._flows.items() if float(v.last_seen) < cutoff]
        for k in stale:
    ...
    def _prune_raw_packets_locked(self, now: float) -> None:
        ...
        while self._raw_packet_order:
            pid = self._raw_packet_order[0]
            item = self._raw_packets.get(pid)
            if item is None:
                self._raw_packet_order.popleft()
                continue
            _, ts = item
            if ts >= cutoff and self._raw_packet_total_bytes <= self._raw_packet_budget_bytes:
                break
            self._raw_packet_order.popleft()
            raw, _ = self._raw_packets.pop(pid, (b"", 0.0))
            self._raw_packet_total_bytes = max(0, self._raw_packet_total_bytes - len(raw))
    ...
    def _store_raw_packet_locked(self, rec: Dict[str, Any], raw: Optional[bytes]) -> None:
        ...
        pid = int(rec["id"])
        existing = self._raw_packets.get(pid)
        if existing is not None:
            self._raw_packet_total_bytes = max(0, self._raw_packet_total_bytes - len(existing[0]))
        self._raw_packets[pid] = (b, float(rec.get("ts") or time.time()))
        self._raw_packet_order.append(pid)
        self._raw_packet_total_bytes += len(b)
        while self._raw_packet_total_bytes > self._raw_packet_budget_bytes and self._raw_packet_order:
            drop_id = self._raw_packet_order.popleft()
            entry = self._raw_packets.pop(drop_id, None)
            if entry is None:
                continue
            self._raw_packet_total_bytes = max(0, self._raw_packet_total_bytes - len(entry[0]))
            self._packet_counters["dropped_raw_packets"] += 1
            self._packet_counters["dropped_raw_bytes"] += len(entry[0])
        self._packet_counters["raw_packet_count"] = len(self._raw_packets)
        self._packet_counters["raw_packet_bytes"] = self._raw_packet_total_bytes

    def _prune_client_packets_locked(self, now: float) -> None:
        cutoff = now - self.client_stream_ttl_sec

        while len(self._client_packet_meta) > self.client_stream_max_packets:
            oldest = self._client_packet_meta.popleft()
            old_id = int(oldest.get("id") or 0)
            entry = self._client_packet_raw.pop(old_id, None)
            if entry is not None:
                self._client_packet_total_bytes = max(0, self._client_packet_total_bytes - len(entry[0]))
                self._packet_counters["dropped_client_packets"] += 1
                self._packet_counters["dropped_client_bytes"] += len(entry[0])

        while self._client_packet_order:
            pid = self._client_packet_order[0]
            entry = self._client_packet_raw.get(pid)
            if entry is None:
                self._client_packet_order.popleft()
                continue
            _, ts = entry
            if ts >= cutoff and self._client_packet_total_bytes <= self.client_stream_max_bytes:
                break
            self._client_packet_order.popleft()
            dropped = self._client_packet_raw.pop(pid, None)
            if dropped is None:
                continue
            self._client_packet_total_bytes = max(0, self._client_packet_total_bytes - len(dropped[0]))
            self._packet_counters["dropped_client_packets"] += 1
            self._packet_counters["dropped_client_bytes"] += len(dropped[0])

        while self._client_packet_meta and int(self._client_packet_meta[0].get("id") or 0) not in self._client_packet_raw:
            self._client_packet_meta.popleft()

        self._packet_counters["client_packet_count"] = len(self._client_packet_raw)
        self._packet_counters["client_packet_bytes"] = self._client_packet_total_bytes

    def _store_client_packet_locked(self, rec: Dict[str, Any], raw: Optional[bytes]) -> None:
        if not raw:
            return
        pid = int(rec["id"])
        b = bytes(raw)
        existing = self._client_packet_raw.get(pid)
        if existing is not None:
            self._client_packet_total_bytes = max(0, self._client_packet_total_bytes - len(existing[0]))
        self._client_packet_raw[pid] = (b, float(rec.get("ts") or time.time()))
        self._client_packet_order.append(pid)
        self._client_packet_total_bytes += len(b)

        meta = dict(rec)
        meta["client_raw_len"] = len(b)
        self._client_packet_meta.append(meta)
        self._prune_client_packets_locked(float(rec.get("ts") or time.time()))

    def get_client_packet_record(self, packet_id: int) -> Optional[Dict[str, Any]]:
        pid = int(packet_id)
        with self._lock:
            for item in self._client_packet_meta:
    ...
    def _classify_packet(
        ...
        *,
        ...
        pkt, raw, quality = self._coerce_packet(packet, raw_bytes=raw_bytes)
        now = time.time()
        ts_ns = time.time_ns()
        ...
        l3 = "unknown"
        l4 = "unknown"
        wire_len = len(raw) if raw is not None else 0
        ...
        if pkt is not None:
            try:
                ...
                summary = type(pkt).__name__
            try:
                if hasattr(pkt, "src"):
                    src_ip = str(getattr(pkt, "src", None) or None)
                if hasattr(pkt, "dst"):
                    dst_ip = str(getattr(pkt, "dst", None) or None)
            except Exception:
                pass
            try:
                if hasattr(pkt, "sport"):
                    sport = int(getattr(pkt, "sport"))
            except Exception:
                pass
            try:
                if hasattr(pkt, "dport"):
                    dport = int(getattr(pkt, "dport"))
            except Exception:
                pass
            try:
                if hasattr(pkt, "flags"):
                    flags = str(getattr(pkt, "flags"))
            except Exception:
                pass
            proto = self._guess_proto(pkt, sport, dport)
            topic = self._guess_topic(proto, sport, dport, summary)
            l3 = self._guess_l3(pkt, summary)
            l4 = self._guess_l4(pkt, sport, dport, flags, summary)
        else:
            summary = f"raw packet ({len(raw or b'')} bytes)"
            proto = self._guess_topic("unknown", None, None, summary)
            topic = proto
            if raw is not None:
                wire_len = len(raw)

        rec = PacketRecord(
            id=self._reserve_packet_id(),
            ts=now,
            ts_ns=ts_ns,
    ...
    def _coerce_packet(self, packet: Any, *, raw_bytes: Optional[bytes]) -> Tuple[Any, Optional[bytes], str]:
        ...
        if isinstance(packet, (bytes, bytearray)):
            return None, bytes(packet), "raw_only"
        try:
            raw = bytes(packet)
            return packet, raw, "native_bytes"
        except Exception:
            return packet, None, "object_only"

    def _guess_proto(self, pkt: Any, sport: Optional[int], dport: Optional[int]) -> str:
        summary = ""
        try:
            summary = str(pkt.summary()).lower()
        except Exception:
            pass
        if sport in self.SPECIAL_PORT_TOPICS:
            return self.SPECIAL_PORT_TOPICS[sport]
        if dport in self.SPECIAL_PORT_TOPICS:
            return self.SPECIAL_PORT_TOPICS[dport]
        for token in ("tcp", "udp", "icmp", "icmpv6", "arp", "ipv6"):
            if token in summary:
                return token
        return type(pkt).__name__.lower() if pkt is not None else "unknown"

    def _guess_topic(self, proto: str, sport: Optional[int], dport: Optional[int], summary: str) -> str:
        if sport in self.SPECIAL_PORT_TOPICS:
            return self.SPECIAL_PORT_TOPICS[sport]
        if dport in self.SPECIAL_PORT_TOPICS:
            return self.SPECIAL_PORT_TOPICS[dport]
        proto_l = str(proto or "unknown").lower()
        if proto_l in self.IMPORTANT_TOPICS:
            return proto_l
        summary_l = str(summary or "").lower()
        for token in self.IMPORTANT_TOPICS:
            if token in summary_l:
                return token
        return proto_l or "unknown"

    def _guess_l3(self, pkt: Any, summary: str) -> str:
        s = str(summary or "").lower()
        for token in ("ipv6", "ip", "arp"):
            if token in s:
                return token
        return "unknown"

    def _guess_l4(self, pkt: Any, sport: Optional[int], dport: Optional[int], flags: Optional[str], summary: str) -> str:
        s = str(summary or "").lower()
        for token in ("tcp", "udp", "icmp", "icmpv6", "esp"):
            if token in s:
                return token
        if sport is not None or dport is not None or flags is not None:
            return "transport"
        return "unknown"
...
<head>
  ...
  <style>
    body { background:#0d1117; color:#c9d1d9; font:14px/1.4 system-ui, sans-serif; margin:0; }
    header { padding:16px 20px; border-bottom:1px solid #30363d; position:sticky; top:0; background:#0d1117; }
    .grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(260px,1fr)); gap:14px; padding:16px 20px; }
    .card { background:#161b22; border:1px solid #30363d; border-radius:14px; padding:14px; box-shadow:0 8px 30px rgba(0,0,0,.20); }
    .mono { font-family: ui-monospace, SFMono-Regular, Menlo, monospace; }
    #logs { height: 50vh; overflow:auto; white-space:pre-wrap; background:#0b0f14; border:1px solid #30363d; border-radius:14px; padding:12px; }
    .ok { color:#3fb950; }
    .bad { color:#f85149; }
    .warn { color:#d29922; }
    .kv { display:flex; justify-content:space-between; gap:8px; margin:4px 0; }
    .tag { display:inline-block; padding:2px 8px; border-radius:999px; background:#1f2937; margin-right:6px; }
    button, select, input { background:#111827; color:#c9d1d9; border:1px solid #30363d; border-radius:10px; padding:8px 10px; }
  </style>
</head>
<body>
  <header>
    <div style=\"display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap\">
      <div>
        <div style=\"font-size:22px;font-weight:700\">__DASHBOARD_TITLE__</div>
        <div id=\"status\" class=\"mono warn\">Connecting…</div>
      </div>
      <div style=\"display:flex;gap:8px;align-items:center;flex-wrap:wrap\">
        <label>Prefix <select id=\"prefix\"><option value=\"\">All</option></select></label>
        <label>Contains <input id=\"contains\" placeholder=\"filter text\" /></label>
  ...
  <div class=\"grid\">
    <div class=\"card\"><div style=\"font-weight:700;margin-bottom:8px\">Server</div><div id=\"server\"></div></div>
    <div class=\"card\"><div style=\"font-weight:700;margin-bottom:8px\">Packets</div><div id=\"packets\"></div></div>
    <div class=\"card\"><div style=\"font-weight:700;margin-bottom:8px\">Logs</div><div id=\"logstats\"></div></div>
    <div class=\"card\"><div style=\"font-weight:700;margin-bottom:8px\">Router</div><div id=\"router\"></div></div>
  </div>

  <div style=\"padding:0 20px 20px 20px\">
    <div class=\"card\">
      <div style=\"display:flex;justify-content:space-between;align-items:center;margin-bottom:8px\">
        <div style=\"font-weight:700\">Live logs</div>
...
let lastLogId = 0;
let lastPrefixesHash = '';

function esc(v) {
  return String(v ?? '').replace(/[&<>\"]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[c]));
}
function kv(obj) {
  return Object.entries(obj || {}).map(([k,v]) => `<div class=\"kv\"><span>${esc(k)}</span><span class=\"mono\">${esc(v)}</span></div>`).join('');
}
function counterList(items) {
  return (items || []).slice(0,10).map(x => `<div class=\"kv\"><span>${esc(x.key)}</span><span class=\"mono\">${esc(x.count)}</span></div>`).join('');
}
function colorForLevel(level) {
  level = String(level || '').toLowerCase();
  if (level === 'error') return '#f85149';
  if (level === 'warning' || level === 'warn') return '#d29922';
  if (level === 'debug') return '#58a6ff';
  return '#c9d1d9';
}
function appendLogs(items) {
  const atBottom = Math.abs(els.logs.scrollHeight - els.logs.scrollTop - els.logs.clientHeight) < 12;
  const html = items.map(item => {
    const ts = new Date((item.ts || 0) * 1000).toLocaleTimeString();
    const repeat = (item.repeat_count || 1) > 1 ? ` ×${item.repeat_count}` : '';
    return `<div style=\"color:${colorForLevel(item.level)}\">[${esc(ts)}] ${esc(item.message)}${esc(repeat)}</div>`;
  }).join('');
  if (items.length) els.logs.insertAdjacentHTML('beforeend', html);
  if (atBottom) els.logs.scrollTop = els.logs.scrollHeight;
}
async function reloadStats() {
  const res = await fetch('/api/stats', {cache:'no-store'});
  const data = await res.json();
  const workerOk = data.server.http_worker_alive && data.server.housekeeping_alive;
  els.status.className = 'mono ' + (workerOk ? 'ok' : 'bad');
  els.status.textContent = workerOk
    ? `Healthy • restarts=${data.server.supervisor_restarts} • snapshot_age=${data.server.snapshot_age_sec || 0}s`
    : `Degraded • restarts=${data.server.supervisor_restarts} • check health`; 
  els.server.innerHTML = kv({
    uptime_sec: data.server.uptime_sec,
    http_worker_alive: data.server.http_worker_alive,
    supervisor_alive: data.server.supervisor_alive,
    housekeeping_alive: data.server.housekeeping_alive,
    supervisor_restarts: data.server.supervisor_restarts,
    consecutive_worker_failures: data.server.consecutive_worker_failures,
  });
  els.packets.innerHTML = kv({
    total_packets: data.packets.total_packets,
    recent_packets_per_sec: data.packets.recent_packets_per_sec,
    recent_bytes_per_sec: data.packets.recent_bytes_per_sec,
    raw_packet_count: data.packets.raw_packet_count,
    raw_packet_bytes: data.packets.raw_packet_bytes,
  }) + '<hr style=\"border-color:#30363d\">' + counterList(data.packets.by_topic);
  els.logstats.innerHTML = kv({
    total_logs: data.logs.total_logs,
    coalesced_logs: data.logs.coalesced_logs,
    live_log_records: data.logs.live_log_records,
    active_prefix_buckets: data.logs.active_prefix_buckets,
  }) + '<hr style=\"border-color:#30363d\">' + counterList(data.logs.by_prefix.map(x => ({key:x.prefix, count:x.count})));
  els.router.innerHTML = kv(data.router || {});
  const prefixItems = data.logs.by_prefix || [];
  const hash = JSON.stringify(prefixItems.map(x => [x.prefix, x.count]));
  if (hash !== lastPrefixesHash) {
    lastPrefixesHash = hash;
    const current = els.prefix.value;
    els.prefix.innerHTML = '<option value=\"\">All</option>' + prefixItems.map(x => `<option value=\"${esc(x.prefix)}\">${esc(x.prefix)} (${x.count})</option>`).join('');
    els.prefix.value = current;
  }
}
async function reloadLogs(reset=false) {
  if (reset) {
    lastLogId = 0;
  ...
  qs.set('limit', '500');
  qs.set('after_id', String(lastLogId));
  if (els.prefix.value) qs.set('prefix', els.prefix.value);
  if (els.contains.value.trim()) qs.set('contains', els.contains.value.trim());
  qs.set('sort_by', 'time_asc');
  const res = await fetch('/api/logs?' + qs.toString(), {cache:'no-store'});
  const data = await res.json();
  const items = data.items || [];
  appendLogs(items);
  for (const item of items) lastLogId = Math.max(lastLogId, Number(item.id || 0));
  els.logmeta.textContent = `showing ${items.length} new • oldest=${data.oldest_id || 0} newest=${data.newest_id || 0}`;
}
async function boot() {
  await reloadStats();
  await reloadLogs(true);
  setInterval(async () => { try { await reloadStats(); } catch (err) { els.status.className='mono bad'; els.status.textContent='Stats reconnecting…'; } }, 2000);
  setInterval(async () => { try { await reloadLogs(false); } catch (err) {} }, 1000);
}
els.reload.addEventListener('click', async () => { await reloadStats(); await reloadLogs(true); });
els.prefix.addEventListener('change', async () => { await reloadLogs(true); });
els.contains.addEventListener('change', async () => { await reloadLogs(true); });
boot();
</script>
</body>
</html>
""".replace("__DASHBOARD_TITLE__", self.dashboard_title)
